...
class Triathlon extends Component{
    ...
    update(cell, row,extra){
        ...
        if((clubAdmin == row.club_id) || admin) {
            return (
                <UpdateTriathlon events={extra[0]} fighter={row}></UpdateTriathlon>
            )
        }
    }
    ...
    render(){
        ...
        let fighters = _.orderBy(this.props.fighters,['triathlonTable.points'],['desc']);
        return(
            <div>
                ...
                <BootstrapTable containerClass="table-responsive-custom" tableContainerClass="table-responsive-custom" data={ fighters } hover pagination multiColumnSort={ 2 }>
                    ...
                    <TableHeaderColumn width="40" dataField='triathlonTable'  dataFormat={ this.format} formatExtraData="win" caretRender={this.renderCaret} dataSort={ true }>Win</TableHeaderColumn>
                    <TableHeaderColumn width="40" dataField='triathlonTable'  dataFormat={ this.format} formatExtraData="loss" caretRender={this.renderCaret} dataSort={ true }>Lost</TableHeaderColumn>
                    {(admin || clubAdmin) &&
                    <TableHeaderColumn width="50" dataField='triathlonTable' dataFormat={ this.update}
