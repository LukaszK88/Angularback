...
class Polearm extends Component{
    ...
    update(cell, row,extra){
        ...
        if((clubAdmin == row.club_id) || admin) {
            return (
                <UpdatePolearm events={extra[0]} fighter={row}></UpdatePolearm>
            )
        }
    }
    ...
    render(){
        ...
        let fighters = _.orderBy(this.props.fighters,['polearmTable.points'],['desc']);
        return(
            <div>
                ...
                <BootstrapTable containerClass="table-responsive-custom" tableContainerClass="table-responsive-custom" data={ fighters } hover pagination multiColumnSort={ 2 }>
                    ...
                    <TableHeaderColumn width="40" dataField='polearmTable'  dataFormat={ this.format} formatExtraData="win" caretRender={this.renderCaret} dataSort={ true }>Win</TableHeaderColumn>
                    <TableHeaderColumn width="40" dataField='polearmTable'  dataFormat={ this.format} formatExtraData="loss" caretRender={this.renderCaret} dataSort={ true }>Lost</TableHeaderColumn>
                    {(admin || clubAdmin) &&
                    <TableHeaderColumn width="50" dataField='polearmTable' dataFormat={ this.update}
