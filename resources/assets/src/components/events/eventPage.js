...
class EventPage extends Component{
    ...
    componentWillUpdate(nextProps){
        if(this.state.attending == false && nextProps.events.event != null && nextProps.currentUser.isLoggedIn) {
            const isUserAttending = _.find(nextProps.events.event.attendance, (attendance) => {
                return attendance.user_id == nextProps.currentUser.user.id;
            });
            if (isUserAttending) {
    ...
    renderCategories(){
        ...
        const attendees = _.filter(event.attendance, (attendee) =>{
            return attendee.event_attend_category != null;
        });
        ...
        return _.map(event.category,(category) => {
            const count =  _.filter(attendedCategories,(cat) => {
