...
import { FETCH_EVENTS, FETCH_EVENT_TYPES,DELETE_EVENT,ADD_EVENT,FETCH_USER_EVENTS,FETCH_EVENT,FETCH_EVENTS_BY_TYPE } from './types';
...
export function notGoingToEvent(eventId,userId) {
    return axios.delete(`${API}event-attend/${eventId}/${userId}`).then((response) => {
        return (dispatch) => {
...
export function attendEvent(data,eventId,userId) {
    return axios.post(`${API}event-attend/${eventId}/${userId}`,data).then((response) => {
        return (dispatch) => {
