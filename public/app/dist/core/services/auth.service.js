        ...
        function Auth($auth, $http, $q, config) {
            ...
            this.currentUser = function () {
                if (_this.$auth.getToken()) {
                    return _this.$http.get(_this.config.API + 'user-current');
                }
                else {
            ...
            this.updateUser = function (data) {
                if (_this.$auth.getToken()) {
                    return _this.$http.put(_this.config.API + 'user/update', data);
                }
                else {
                    _this.$q.reject({ message: 'User has no Token' });
                }
            ...
            this.updateUserPassword = function (data) {
                if (_this.$auth.getToken()) {
                    return _this.$http.put(_this.config.API + 'user/updatePassword', data);
                }
                else {
                    _this.$q.reject({ message: 'User has no Token' });
                }
